...
async function confirmBuy() {
  ...
  try {
    ...
    const res = await fetch(
      `${API}/${currentListing._id}/purchase`,
      {
        method:"POST",
        headers:{
        ...
        body:JSON.stringify({ kwh: buyQty })
      }
    );
    ...
    Session.set({ walletBalance: data.remainingWallet });

    cartCount++;
    document.getElementById("cartN").textContent =
      cartCount;
    ...
    showToast(
      "Purchase Successful ✅",
      `${buyQty} kWh purchased • Wallet ₹${(data.remainingWallet || 0).toFixed(2)}`
    );
